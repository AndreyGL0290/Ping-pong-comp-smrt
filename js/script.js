...
//Считаем очки
let score = 0;
document.getElementById("counter").textContent = "Твой результат: 0";
//Переменная для лучшего результата
let best_score = 0;
document.getElementById("best_score").textContent = "Твой лучший результат: 0";
...
//Главный цикл игры
function loop(){
    leftPaddle.y = ball.y - leftPaddle.height / 2 + grid / 2;
    //Если мяч вылетел за границы переигрываем
    if (ball.x > rightPaddle.x){
        //Если счет в игре больше лучшего результата - перезаписываем его
        if (score > best_score){
            best_score = score;
            document.getElementById("best_score").textContent = 'Твой лучший результат: ' + best_score;
        }
        ...
        leftPaddle.y = ball.y;
        score = 0
        document.getElementById("counter").textContent = 'Твой счет: ' + score;
        setTimeout(() => {
            //Число определяющее полет мяча
